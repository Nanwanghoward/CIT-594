...
/**
 * The Computer class - It implements the player interface as a computer player
 * not a user player. It will has methods from Player interface
 * 
 * @author Hany
 *
 */
public class Computer implements Player {
  ...
  /**
   * This method calls a series of method to get the location of ship we want to
   * put. This method will place a ship on the grid. This method should
   * guarantee correctness of location (no overlaps, no ships over the edge of
   * the board, etc.)
   * 
   * @param size
   *          the size of the ship to place
   * @param retry
   *          if an earlier call to this method returned an invalid position,
   *          this method will be called again with retry set to true.
   ...
   *          the size of the ship to place.
   * @param x
   *          the x coordinate of ship location.
   * @param y
   *          the x coordinate of ship location.
   * @param isHorizontal
   *          the horizon status of ship.
   ...
   * 
   * @param lo
   *          The location the ship we want to place on board.
   * @param size
   *          the size of ship we want to put on board.
   */
  ...
  /**
   * This method will get a new target. The Player can choose whichever location
   * it wants to aim for next and return that location.
   * 
  ...
  /**
   * This method will find the location of target we want to fire based on the
   * previous fire location we hit the ship.
   * 
  ...
  private Location findLo() {
    ...
    if (validate(list.get(i).getX(), list.get(i).getY()) != 0) {
      return null;
    }
    return list.get(i);
  }
  ...
  /**
   * This method checks status of the other player's board in the location of
   * coordinate x and y.
   * 
   * @param x
   *          The x coordinate of target we want to fire.
   * @param y
   *          The y coordinate of target we want to fire.
   * @return return -2 if the location of target out of board, -1 if already
   *         hit, 0 if not fired, 1 if hit, 2 if sunk.
   */
